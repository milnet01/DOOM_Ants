...
- `linuxdoom-1.10/rb_image.h` / `rb_image.c` — thin decode+downscale wrapper around stb.
- `linuxdoom-1.10/rb_materials.h` / `rb_materials.c` — sidecar parse, name→id resolution, per-id control table, traffic/budget decisions (pure CPU; unit-tested).
...
- `linuxdoom-1.10/Makefile` — add `rb_image.o`, `rb_materials.o` to `OBJS`.
...
## Task 2: Sidecar parser — rows + flags (pure C, unit-tested)

**Files:**
- Create: `linuxdoom-1.10/rb_materials.h`
- Create: `linuxdoom-1.10/rb_materials.c`
...
int          rb_parse_material_line(const char* line, rb_matrow_t* out);
unsigned int rb_parse_flags(const char* field);
...
`linuxdoom-1.10/rb_materials.c` (this task adds the parser; later tasks append to this file):
```c
#include "rb_materials.h"
#include <string.h>
#include <stdlib.h>
...
unsigned int rb_parse_flags(const char* field) {
    ...
    for (char* t = strtok_r(buf, "|", &save); t; t = strtok_r(NULL, "|", &save)) {
        char* tok = trim(t);
        if (!*tok)                    continue;
        else if (!strcmp(tok, "pom"))    f |= RB_FLAG_POM;
...
int rb_parse_material_line(const char* line, rb_matrow_t* out) {
    ...
    /* Split into exactly 11 positional columns on ',' (empty cells preserved). */
    char* col[11];
    int n = 0;
    char* p = buf;                              /* re-walk raw buf (s pointed into it) */
    s = trim(buf);
    /* strtok collapses empty fields, so hand-split to keep blank cells. */
    col[n++] = s;
    for (char* c = s; *c; c++) {
    ...
    char* name = trim(col[0]);
    strncpy(out->name, name, 8); out->name[8] = '\0';
    ...
    for (int i = 0; i < RB_MAP_COUNT; i++) {
        char* m = trim(col[2 + i]);
        strncpy(out->maps[i], m, sizeof(out->maps[i]) - 1);
    }
    out->uv_scale = (float)atof(trim(col[9]));
    if (out->uv_scale <= 0.0f) out->uv_scale = 1.0f;   /* blank/invalid -> 1.0 */

    out->flags = rb_parse_flags(trim(col[10]));
...
```bash
git add linuxdoom-1.10/rb_materials.h linuxdoom-1.10/rb_materials.c linuxdoom-1.10/tests/rb_materials_test.cpp
git commit -m "DOOM-0042: sidecar row + flags parser (rb_materials)
...
**Files:**
- Modify: `linuxdoom-1.10/rb_materials.h` (append)
- Modify: `linuxdoom-1.10/rb_materials.c` (append)
...
void rb_build_ctrl_table(const rb_matrow_t* rows, int nrows, int nmaterials,
...
// Minimal 2x2 RGBA PNG written to a temp file, then decoded + downscaled.
static const char* write_tmp_png() {
    // A 2x2 red/green/blue/white PNG, hand-built (filter 0 per row, zlib stored).
    // Simpler: shell out to python for the fixture (build env has python3).
    static char path[] = "/tmp/rb_image_test_2x2.png";
    system("python3 -c \""
           "import struct,zlib\n"
           ...
           "for y in range(2):\n raw.append(0)\n"
           " for x in range(2):\n  r,g,b=px[y*2+x];raw+=bytes((r,g,b))\n"
           "png=b'\\x89PNG\\r\\n\\x1a\\n'+ch(b'IHDR',struct.pack('>IIBBBBB',2,2,8,2,0,0,0))"
           "+ch(b'IDAT',zlib.compress(bytes(raw),9))+ch(b'IEND',b'')\n"
           "open('/tmp/rb_image_test_2x2.png','wb').write(png)\"");
    return path;
}
...
int main() {
    ...
    const char* p = write_tmp_png();
    assert(rb_image_load(p, &img) == 1);
    assert(img.w == 2 && img.h == 2);
    // top-left is red, full alpha (stb fills alpha=255 for RGB source)
    assert(img.pixels[0] == 255 && img.pixels[1] == 0 && img.pixels[2] == 0 && img.pixels[3] == 255);

    rb_image_downscale_max(&img, 1);        // 2x2 -> 1x1, average of the four corners
    assert(img.w == 1 && img.h == 1);
    // average of red/green/blue/white = (128,128,128) approx (rounding tolerant)
    assert(img.pixels[0] >= 120 && img.pixels[0] <= 136);
    rb_image_free(&img);
...
Expected: FAIL — `rb_image.h: No such file or directory`.

- [ ] **Step 4: Write the wrapper header + impl**

`linuxdoom-1.10/rb_image.h`:
```c
#ifndef RB_IMAGE_H
#define RB_IMAGE_H
...
- [ ] **Step 3: Append the header decl**

```c
void rb_apply_budget(rb_matctrl_t* table, int nmaterials,
...
```c
#include <stdlib.h>   /* qsort (already included above; harmless if repeated) */

typedef struct { int id; float traffic; int is_hero; } rb_sortent_t;

static int rb_cmp_ent(const void* a, const void* b) {
...
- Produces: `g.hdSampler` — a `VkSampler` with `VK_FILTER_LINEAR` min/mag, `VK_SAMPLER_MIPMAP_MODE_LINEAR`, `VK_SAMPLER_ADDRESS_MODE_REPEAT` on U/V/W, `maxLod = VK_LOD_CLAMP_NONE`; the HD-array counterpart to the nearest paletted `g.texSampler`. Also `rb_image.o` + `rb_materials.o` linked into the engine.

- [ ] **Step 1: Add the two objects to OBJS**
...
Expected: links `linux/linuxxdoom` clean; `rb_image.o` + `rb_materials.o` compiled; no new warnings.
...
Include `rb_materials.h` at the top of `r_vulkan.cpp` inside an `extern "C" { }` block (it is a C header).
...
- Modify: `linuxdoom-1.10/rb_materials.h` / `.c` (a `rb_traffic_from_mesh` helper + `rb_asset_path` join)
...
- [ ] **Step 1: Add the asset-root + traffic helpers to rb_materials**

Header:
```c
const char* rb_asset_root(void);          /* DOOMASSETDIR or "assets/ultra/" */
void        rb_asset_path(char* dst, int dstsz, const char* rel);  /* root + '/' + rel */
```
Impl:
```c
#include <stdio.h>
const char* rb_asset_root(void) {
    const char* e = getenv("DOOMASSETDIR");
    return (e && *e) ? e : "assets/ultra/";
}
void rb_asset_path(char* dst, int dstsz, const char* rel) {
...
```
Traffic from mesh (sum triangle area per unified id). The engine already has the mesh; pass it in:
```c
/* Accumulate world surface area per unified material id from the rb_vertex_t soup.
   ...
   flat_flag = the FLAG_FLAT bit; num_wall/num_flat map texnum -> unified id. */
void rb_traffic_from_mesh(const float* verts, int nverts, int stride_floats,
                          unsigned int flag_flat, int num_wall, int num_flat,
                          int nmaterials, float* traffic_out);
```
(Implementation: for each triangle `t` (verts `3t,3t+1,3t+2`), area = ½|(p1−p0)×(p2−p0)|; unified id from vertex 0's texnum/flags exactly as `pathtrace.comp:439-440`; `traffic_out[id] += area`. Zero-init `traffic_out` first. Guard ids into `[0,nmaterials)`.)
...
```bash
git add linuxdoom-1.10/r_vulkan.cpp linuxdoom-1.10/rb_materials.h linuxdoom-1.10/rb_materials.c
