...
- **DOOM-0042** (Ultra HD PBR materials) — provides the HD material set this
  ...
  (`pathtrace.comp`: `ctrl[]` SSBO L79, `hdBaseUV` :142, `hdParallaxUV` :175,
  `hdAlbedo` :206, `hdShadingNormal` :219, `hdAO` :230).
...
| Ultra with RT **on** (`rb_rtdebug` 4/6) | path tracer | **Yes** — de-tiling on HD `usePBR` maps; filth on all non-sprite surfaces |
...
border blend does not cover the march. **Border-band skip (as-built, shipped in
`detilePOM`, `pathtrace.comp:563`):** inside a border band around each cell edge —
`min(min(f.x, 1−f.x), min(f.y, 1−f.y)) < 0.1` — the parallax march is skipped and
the de-tiled `baseUV` is sampled directly (the wrappers still de-tile it), *not*
the pre-de-tile coordinate. The `0.1` band width is the §10 Q1 tuning knob.
...
- **Two new image kinds** (11 files total; as-built — revises the spec's original
  "no new images"), all ordinary `hdTex[]` entries, no new binding:
  - `assets/ultra/overlays/dirt.png` — one global dirt **colour** overlay (sRGB),
    first-party CC0, generated by `scripts/make_dirt.py` (FFT fractal noise →
    warm-brown palette). Loaded as a global overlay in `EnsureHdMaterials`
    (`r_vulkan.cpp:5139`–`5172`, alongside the DOOM-0179 grunge overlay); rides to
    the trace as `misc5.z`; missing/undecodable leaves it off (`hdDirtIdx = -1` →
    `0xFFFFFFFF`), and `stainColour` falls back to the procedural earthy ramp.
  - 10 baked hero **AO** maps (`*_ao.png`, one per CC0 hero), produced by
    `scripts/hero_ao.py` from each hero's `_hgt.png` relief. These load through the
    normal per-material HD path (not the overlay block above) and feed the crevice
...
- **Tuning knobs (compile-time `const`s, like `kGrimeStrength`/`kGrimeWorldScale`).**
  ...
  exposes no named width knob — its weights are `smoothstep(0.25, 0.75, f)` over the
  fractional cell position `f` (`detile`, `pathtrace.comp:503`), an inlined band
  rather than a `const`.
- **Runtime dial (as-built).** The `misc5.y` lane is the de-tile quality/enable dial
  (see the lane map above: 0 = off, 1 = 2-tap, 2 = 4-tap; `]` key). Note the
  "4-tap albedo + cheaper other maps" split (§6) is a **compile-time** variant, not
  a separate `misc5.y` value. The lane rides on the mode-5 verify struct's `misc5`
  padding, so `-rtverify` is unaffected (INV-9).

## 6. Performance budget

- **Baseline (L5 method).** The perf reference was the RT-on E1M1 Ultra frame rate
  — path tracer, modes 4/6 — at 50 % render scale with flashlight, with de-tiling
  **off** (`misc5.y = 0`). This is **not** the ~65 FPS DOOM-0170 raster
...
*As-built: all layers L1–L5 were built and user-accepted (last sign-off
2026-07-17); the table below is the historical build record.* Each layer was
independently play-tested for user acceptance (renderer look is a play-test call).
L1–L4 acceptance was **human play-test only** — no automated proxy metric, matching
the renderer-look convention (DOOM-0179 / DOOM-0042); only **L5**'s ≤ 5 % perf check
(§6) was objective. L1–L4 FPS numbers were diagnostic.
...
- **Q1 (POM):** *Resolved.* POM is marched in de-tiled space (§4.4), **with** the
