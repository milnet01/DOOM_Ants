...
- **B7 — the CPU blind-spot counters.** `tick`, `overlay`, `sound`, `wipe`.
  *Verify:* the gap is `frame,total` minus `cpu,present-total` — that pair, not
  a sum of the `cpu` rows, which §4.1 forbids because `cpu,build` is depth 0 and
  may already sit inside `present-total`. Measure it before B7, then require the
  four new rows to account for at least half of it.
...
- **`docs/specs/DOOM-0345-bloom-ray-traced.md` INV-7** — constrains the GPU
  ...
  constraint into the new name tables. **But an edit IS owed**, and it is not
  about slots: that invariant's standing test greps `r_vulkan.cpp` for the
  literal `nq = g.profRasterFrame ? 7u : 10u;` and expects one hit. §4.2
  rewrites that line to read `kRasterTimestamps` / `kRtTimestamps`, so the grep
  would return zero and a live test on a shipped feature would read as broken.
  The grep is updated to the constant form in the same change that rewrites the
  line — B2 carries it.
...
| 2-plan2 | 2026-09-12 | 0 | 0 | 0 | 0 | 0 | **No reviewer was dispatched against this document.** Second fold-back from the plan's gate, loop 2. Two amendments. §7 B7 still prescribed "`frame,total` minus the sum of the `cpu` rows", which §4.1 of this same document forbids by name because `cpu,build` is depth 0 and may already sit inside `present-total`; the plan's loop 1 fixed its own copy and left this one, and two lanes found the divergence. It now states the `frame,total` minus `cpu,present-total` pairing, with a floor the new rows must clear. §12 said "No edit owed" of DOOM-0345 — true about timestamp slots, false about that spec's INV-7 standing grep, which matches the literal `nq = g.profRasterFrame ? 7u : 10u;` that §4.2 rewrites. §12 now records the edit as owed and B2 carries it. **Not re-gated**, on §At the cap's routing for a violent cap; see the `2-plan` row. |
