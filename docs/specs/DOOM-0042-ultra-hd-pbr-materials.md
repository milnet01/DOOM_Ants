...
- **Height / POM (option a):** for `flags:pom` materials, ray-march the height field along the **view direction in tangent space** and offset the UV so relief recesses and self-occludes *in the primary-hit shade only*. **Bounded march:** **16 linear steps at normal incidence, scaling up to 32 at grazing angles** (step count ramps linearly in `1 − N·V`), then one binary-search refinement. POM treats **white = raised** (matching the derive generator's brighter=raised, so hero and derived height march the same way). **Bound the parallax _offset_, not the coordinate:** cap the offset magnitude to the height scale and sample through the **REPEAT** sampler, so a step within the periodic height field wraps correctly. Do **not** absolute-clamp the final UV to `[0,1]` — DOOM wall U tiles `0→N` across a long wall, so a `[0,1]` clamp would pin every tile past the first and break POM on the common case. "Degrades gracefully at grazing angles" = the bounded offset + step cap (no gaping holes), measured by the parallax check in Verification. `noPom`/flat-decal materials skip the march entirely. **Known limit (accepted for v1):** the displaced relief is invisible to RT shadow rays and the GI bake — a groove shades but casts no self-shadow. Options (b) height-field occlusion in NEE shadow rays and (c) true displacement-mapped BLAS are explicitly deferred.
