...
- **Normal mapping:** unpack the tangent-space normal (`n = normalize(2·rgb − 1)`, OpenGL **Y+** green-channel with `n.y` un-flipped — ambientCG's default; the derive generator MUST emit the same Y+ so hero and derived normals agree) and rotate it into world space via a tangent frame **derived from the hit triangle**: `tangent = normalize(∂P/∂U)` from the triangle's edge vectors + UV deltas, `bitangent = normal × tangent`. Deriving from `∂P/∂U` yields the correct handedness for walls, floors **and** ceilings automatically (no per-case bitangent flip) and needs no per-vertex tangent attribute in v1. **Sprites are excluded in v1** — a camera-facing billboard has no stable tangent frame, so `flags:sprite` materials use **albedo + alpha only** (normal/roughness/metallic/height not sampled); real sprite relief waits for the DOOM-0080 3D models.
