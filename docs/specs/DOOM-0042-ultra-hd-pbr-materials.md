...
- **PNG decode via a vendored single-header public-domain loader — `stb_image.h`** (PD/MIT, **no new *link* dependency**). Preferred over SDL2_image because only SDL2 + SDL2_mixer are linked today (`linuxdoom-1.10/Makefile:15-16`) — SDL2_image would be a *new* link dep. Record the vendored `stb_image.h` version in the dependencies standard's staged-deps list (`docs/standards/dependencies.md` §"Where this project's dependencies live") so it doesn't silently go stale. Reuse-before-rewrite: only add the loader we actually need.
...
On a surface hit, branch on the material's `usePBR` (read from the control SSBO, §B). `uv_scale` (§A) multiplies the raw UVs **first** (so the height field marches in tiled space); POM (first bullet) then computes the UV offset on those scaled UVs; every map sample below uses that offset UV.
...
- **Roughness / metallic → new GGX specular lobe.** The RT tracer today shades **diffuse-only** — a single Lambert BRDF (`pathtrace.comp:422`); the GGX/VNDF specular lobe DOOM-0009 §4.4 sketches is **deferred, not implemented** ("gated to measured need — matte DOOM art rarely needs it"). HD metal/tech surfaces *are* that measured need, so **DOOM-0042 adds the GGX specular lobe to the RT hit shading** (this is new work, not a pre-existing path): sampled roughness drives the GGX lobe width, metallic drives the F0 / diffuse-vs-specular split, combined with the diffuse term through the MIS slot the §4.4 integrator already reserves. This lobe is the mechanism behind the "wet metal shines" goal and the specular-highlight verification — without it, roughness/metallic would drive nothing under Lambert.
...
| Shading | `pathtrace.comp` (hit decode `:385-399`, albedo via `pt_common.glsl` `decodeAlbedo` `:87-93`, alpha `:184-196`, Lambert BRDF `:422`) | `usePBR` branch: RGBA albedo, normal-map + tangent frame, **new GGX specular lobe** (roughness/metallic), AO on ambient, emissive, POM UV march |
