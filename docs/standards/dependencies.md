...
- `linuxdoom-1.10/assets/Oxanium-SemiBold.ttf` v2.000 (SemiBold weight) — bundled OFL
  font for the crisp Solid/Ultra menu text (DOOM-0206). Fetched from the canonical
  upstream `github.com/sevmeyer/oxanium` (tag `2.000`); ships with its licence at
  `linuxdoom-1.10/assets/OFL.txt`. Embedded into the binary as `oxanium_ttf[]`
  (`assets/Oxanium-SemiBold.ttf.h`, generated by the Makefile's `xxd -n` step). Bump on
  the dependency sweep.
- `branding/doom-logo-source.svg` — the "DOOM" lettering behind the project wordmark,
  from Wikimedia Commons (`File:Doom – Game's logo.svg`), PD-textlogo. Not built or
  shipped; a trademark of id Software. Provenance and credit in `branding/README.md`.
  No version to track.
